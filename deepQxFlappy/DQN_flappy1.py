...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if(playerx < lowerPipes[0]['x']):
            if(playery < upperPipes[0]['y']+pipeH):
                reward = -5
            elif (playery < lowerPipes[0]['y'] and \
                    playery > upperPipes[0]['y']+pipeH):
                if(playery > upperPipes[0]['y']+pipeH+IMAGES['player'][0].get_height()/2):
                    reward = 0.015 + score/10
                else:
                    reward = 0.01 + score/10
            else:
                reward = -10
        elif(playerx < lowerPipes[1]['x']):
            if(playery < upperPipes[1]['y']+pipeH):
                reward = -5
            elif (playery < lowerPipes[1]['y'] and \
                    playery > upperPipes[1]['y']+pipeH):
                if(playery > upperPipes[1]['y']+pipeH+IMAGES['player'][1].get_height()/2):
                    reward = 0.015 + score/10
                else:
                    reward = 0.01 + score/10
            else:
